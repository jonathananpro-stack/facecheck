const CORE = {
    // Đã sửa facingMode: "environment" để kích hoạt Camera Sau
    state: { mode: 'TRAIN', facingMode: "environment", isCombatScanning: false, combatInterval: null, faceMatcher: null, session: { isActive: false, checkedInSet: new Set(), unknownsQueue: [] } },
    ...
    media: {
        async startCamera(vidId) {
            ...
            try {
                ...
                // Gọi camera dựa theo state (Mặc định camera sau)
    ...
    combat: {
        ...
        async scanLoop() {
            ...
            // Đã tăng tốc độ quét lên 60ms cho siêu mượt
            CORE.state.combatInterval = setTimeout(() => this.scanLoop(), 60);
        }
    }
};
