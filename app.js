...
async function initAI() {
    ...
    document.getElementById('setupStatus').innerText = "AI sẵn sàng!";
}
...
document.getElementById('upload').addEventListener('change', async (e) => {
    ...
    // ĐỘ NHẠY TỐI ĐA: inputSize 512, scoreThreshold 0.3
    const detections = await faceapi.detectAllFaces(img, new faceapi.TinyFaceDetectorOptions({ inputSize: 512, scoreThreshold: 0.3 }))
                                     .withFaceLandmarks()
                                     .withFaceDescriptors();

    if (detections.length === 0) return alert("Không tìm thấy khuôn mặt! Hãy thử ảnh rõ nét hơn.");
    ...
    for (let det of detections) {
        ...
        btn.onclick = () => {
            const name = prompt("Tên người này là gì?");
            if (name) {
...
async function startCamera() {
    ...
    video.onloadedmetadata = () => {
        ...
        setInterval(async () => {
            const det = await faceapi.detectSingleFace(video, new faceapi.TinyFaceDetectorOptions({ inputSize: 512, scoreThreshold: 0.3 }))
            ...
            ctx.stroke();
            document.getElementById('status').innerHTML = match ? `<b style="color:green;">${match.name}</b>` : `<b style="color:red;">Người lạ</b>`;
        }, 500);
    };
}
